<?php
			$phrasesGet = DB::query("SELECT * FROM `new_phrases_used` WHERE `status`='1' AND `id_project`='$id_project' AND `id_user`='$id_user' LIMIT 30"); //Релевантные
			...
			if ($check > 0 ) {
				...
				$result .= '	<div class = "videoZag">
									<div class="obolochka">
										<div class="centredv lh32">Список релевантных ключевых запросов</div>
									</div>
								</div>';
				...
				$result .= '	<div class = "videoZag">
									<div class="obolochka">
										<div class="centredv lh32">Нет используемых релевантных ключевых запросов</div>
									</div>
								</div>';
				$result .= '</div>';
			}
